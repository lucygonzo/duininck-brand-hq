...
const INITIAL_ACTIONS: ActionItem[] = [
  ...
  { text: 'Research AI-assisted content production tools Nicole\'s team can adopt (she already uses Claude)', owner: 'Lucy', urgency: 'Medium', category: 'Tools', status: 'open' },
  ...
  return (
    <div>
      ...
      <Block variant="orange">
        <strong>Time-Sensitive:</strong> Golf season starts soon, so golf social must restart NOW. Construction season starts April, meaning the capture calendar must be ready. Centennial event is July 25, so all centennial content needs planning months ahead.
      </Block>
      ...
      {done.length > 0 && (
        <>
          <Divider label={`Completed (${done.length})`} />
          {done.map((a) => {
            const origIdx = actions.indexOf(a);
            return (
